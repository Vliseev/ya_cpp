...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            (tree_values[w].earn *= tree_factor[v]) +=
                tree_add[v] * (r - l) / 2;
            tree_values[w].spend += tree_sub[v] * (r - l) / 2;
        }
    }
    tree_factor[v] = 1;
    tree_add[v] = 0;
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
    ...
    if (ql <= l && r <= qr) {
        tree_add[v] += value;
        tree_values[v].earn += value * (r - l);
        return;
    }
    Add(v * 2, l, (l + r) / 2, ql, qr, value);
    Add(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
    tree_values[v] =
        (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : MoneyState{0.0, 0.0}) +
        (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1]
                                  : MoneyState{0.0, 0.0});
}
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            auto val = ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to);
            cout << val.earn - val.spend << endl;
        } else if (query_type == "PayTax") {
            uint32_t value;
            cin >> value;
            double percent = 1.0 - 1.0 * value / 100.0;
            Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, percent);
        } else if (query_type == "Earn") {
            double value;
            cin >> value;
            Add(1, 0, DAY_COUNT_P2, idx_from, idx_to,
                value / (idx_to - idx_from));
        } else if (query_type == "Spend") {
            double value;
            cin >> value;
            Sub(1, 0, DAY_COUNT_P2, idx_from, idx_to,
                value / (idx_to - idx_from));
        }
    }
