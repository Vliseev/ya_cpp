...
class BulkLinearUpdater {
   public:
    ...
    BulkLinearUpdater(const BulkMoneySubsr& sub) : sub_(sub) {
    }

    void CombineWith(const BulkLinearUpdater& other) {
        tax_.tax_perc *= other.tax_.tax_perc;
    ...
    MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
        return {
            origin.earn * tax_.ComputeFactor() + add_.delta * segment.length(),
            origin.spend + sub_.delta * segment.length()};
    }
...
class SummingSegmentTree {
   public:
    ...
    void AddBulkOperation(IndexSegment segment,
                          const BulkOperation& operation) {
        this->TraverseWithQuery(root_, segment,
                                AddBulkOperationVisitor{operation});
    }
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager& manager) const override {
        auto res = manager.ComputeSum(MakeDateSegment(date_from, date_to));
        return res.earn - res.spend;
    }
...
struct SpendReques : ModifyRequest {
    SpendReques() : ModifyRequest(Type::SPEND) {
    }
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
    ...
    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double daily_income = income * 1.0 / date_segment.length();
        manager.AddBulkOperation(date_segment, BulkMoneySubsr{daily_income});
    }
...
struct PayTaxRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        manager.AddBulkOperation(date_segment,
                                 BulkTaxApplier{1.0 - 1.0 * percent / 100});
    }
...
RequestHolder Request::Create(Request::Type type) {
    switch (type) {
        ...
        case Request::Type::SPEND:
            return make_unique<SpendReques>();
        default:
            return nullptr;
    }
}
...
int main() {
    ...
    istringstream iss(
        "4\n"
        "Earn 2000-01-02 2000-01-06 20\n"
        "PayTax 2000-01-02 2000-01-06 12\n"
        "PayTax 2000-01-02 2000-01-02 11\n"
        "ComputeIncome 2000-01-02 2000-01-03\n");

    cout.precision(25);
    auto requests = ReadRequests(cin);
