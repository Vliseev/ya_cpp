...
void DbManager::AddBus(const std::string& id,
...
size_t DbManager::GetBusStop(const std::string& bus_id) const {
...
size_t DbManager::GetUniqueBusStop(const std::string& bus_id) const {
...
DbManager::GetStopBuses(const std::string& name) const {
    std::vector<std::string_view> result;
    auto stop_it = stops_.find(name);
    if (stop_it == stops_.end()) {
        return result;
    } else {
        for (const auto& bus : stop_it->second->GetBuses())
            result.emplace_back(bus);
        return result;
    }
}

double DbManager::GetRouteLen(const std::string& bus_id) const {
...
bool DbManager::ContainBus(const std::string& bus_id) const {
    return buses_.count(bus_id);
}
bool DbManager::ContainStop(const std::string& stop) const {
