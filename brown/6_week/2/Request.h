...
struct Request {
    enum class Type { BUS_ADD, STOP_ADD, BUS_INFO, STOP_INFO };
...
struct StopInfoRequest : public ReadRequest<std::string> {
    StopInfoRequest() : ReadRequest<std::string>(Type::STOP_INFO){};
    void ParseFrom(std::string_view input) override;
    std::string Process(const DbManager& manager) const override;
    std::string name{};
};
...
std::vector<RequestHolder>
ReadRequests(std::istream& in_stream = std::cin,
